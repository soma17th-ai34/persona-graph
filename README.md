...
검색 결과가 실제 Agent prompt에 들어간 경우, 저장된 run JSON의 `search_records`에 정리된 검색 context가 남습니다. API 키나 raw HTTP 응답은 저장하지 않습니다.
LLM이 사용 가능하면 검색 전에 사용자 입력을 1~3개의 검색엔진 친화 query로 재작성합니다. LLM이 없거나 재작성에 실패하면 기존처럼 원문 query를 fallback으로 사용합니다.
