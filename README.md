...
6. 각 Specialist Agent는 자기 발언 초안을 self verification으로 점검하고, 기준 미달이면 최대 3회까지 refine한 뒤 통과한 답변을 내보냅니다.
7. Moderator Agent가 응답 라운드의 논점을 좁히고, Specialist Agent들이 찬성 측과 반대 측으로 나뉘어 앞선 발언을 강화하거나 반박합니다.
8. Judge Agent가 각 라운드의 찬반 주장을 평가하고 다음 판단 기준을 남깁니다.
9. Critic Agent가 전체 토론의 모순, 약점, 누락을 지적합니다.
10. Synthesizer Agent가 사회자 최종 정리 형식으로 답변을 통합합니다.
11. Evaluator Agent가 최종 결과를 점수와 총평으로 평가합니다.
12. Streamlit UI에서 배너, 데포르메 페르소나 캐릭터, 에이전트 토론 네트워크, 대화창형 토론 로그, 최종 결론, 평가 요약을 표시하고 실행 결과를 저장합니다.
13. 사용자가 대화창 하단에 의견을 입력하면 기본 2명, 최대 3명의 Agent가 답하고 Synthesizer Agent가 현재까지의 중간 결론을 갱신합니다.

## UI Highlights

- 라운드별 토론 흐름 표시: 준비 단계, 오프닝, 첫 의견, 상호 응답, 최종 종합
- 비판 Agent의 내부 검토는 최종 결론에 반영하되 화면 로그에는 노출하지 않음
- 찬성/반대 역할 토론은 표시하되 라운드별 내부 평가는 최종 결론에만 반영
